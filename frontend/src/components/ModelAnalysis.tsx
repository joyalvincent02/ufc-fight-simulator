                    ...
                    <div className="flex-[2] bg-white/80 dark:bg-black/20 p-4 rounded-lg border border-gray-200 dark:border-white/10">
                        ...
                        <div className="grid grid-cols-2 gap-y-3 gap-x-8 text-xs">
                            <div className="flex items-center">
                                ...
                                <span className="text-gray-900 dark:text-white font-medium">{diffs.weight_diff === 0 ? 'Even' : `${diffs.weight_diff > 0 ? fighterA : fighterB} +${Math.abs(diffs.weight_diff)}lbs`}</span>
                            </div>
                            <div className="flex items-center">
                                <span className="text-gray-600 dark:text-gray-300 min-w-[52px]">Height:</span>
                                <span className="text-gray-900 dark:text-white font-medium">{diffs.height_diff === 0 ? 'Even' : `${diffs.height_diff > 0 ? fighterA : fighterB} +${Math.abs(diffs.height_diff)}"`}</span>
                            </div>
                            <div className="flex items-center">
                                <span className="text-gray-600 dark:text-gray-300 min-w-[52px]">Reach:</span>
                                <span className="text-gray-900 dark:text-white font-medium">{diffs.reach_diff === 0 ? 'Even' : `${diffs.reach_diff > 0 ? fighterA : fighterB} +${Math.abs(diffs.reach_diff)}"`}</span>
                            </div>
                            <div className="flex items-center">
                                <span className="text-gray-600 dark:text-gray-300 min-w-[52px]">Age:</span>
                                <span className="text-gray-900 dark:text-white font-medium">{diffs.age_diff === 0 ? 'Even' : `${diffs.age_diff < 0 ? fighterA : fighterB} ${Math.abs(diffs.age_diff)}yr(s) younger`}</span>
                            </div>
                        </div>
                    </div>
                </div>
